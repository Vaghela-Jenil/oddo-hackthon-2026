...
export function InventoryApp() {
        ...
        setProducts((current) => [
          {
            ...
            name: normalizedName,
            sku: normalizedSku,
            category: typeof newProduct.category === 'object' && newProduct.category?.name 
              ? newProduct.category.name 
              : (typeof newProduct.category === 'string' ? newProduct.category : normalizedCategory),
            unit: normalizedUnit,
            reorderPoint: productForm.reorderPoint,
            stockByLocation: {},
          },
  ...
  const addWarehouse = async () => {
    ...
    setIsSubmitting(true);
    try {
      // Call API endpoint to create warehouse in database
      const result = await api.createWarehouse({ name, locations: ["General"] });
      const newWarehouse = result.data || result;
      
      setWarehouses((current) => [...current, { 
        id: newWarehouse.id || `w-${Date.now()}`, 
        name, 
        locations: ["General"] 
      }]);
      setWarehouseNameInput("");
      setBannerMessage(`Warehouse ${name} added successfully.`);
    } catch (error) {
  ...
  const addLocation = async () => {
    ...
    setIsSubmitting(true);
    try {
      // Find the warehouse to get its ID
      const targetWarehouse = warehouses.find((w) => w.name === locationWarehouseInput);
      if (!targetWarehouse) {
        setBannerMessage("Warehouse not found.");
        return;
      }

      // Call API endpoint to add location to warehouse
      await api.addLocationToWarehouse(targetWarehouse.id, { name: location });
      ...
      setBannerMessage(`Location ${location} added to ${locationWarehouseInput} successfully.`);
    } catch (error) {
